...
class _StudentPersonalInfoState extends State<StudentPersonalInfo> {
  ...
  Map<String, bool> seekingKnowledge = {
    "Qaida": false,
    "Recitation": false,
    "Tajweed": false,
    "Hadith": false,
    ...
    try {
      ...
      if (data != null) {
        setState(() {
          ...
          List<dynamic> savedSkills = data['seeking_knowledge'] ?? [];
          for (var skill in savedSkills) {
            if (seekingKnowledge.containsKey(skill)) {
              seekingKnowledge[skill] = true;
            }
          }
        });
      }
  ...
  Future<void> _saveStudentData() async {
    ...
    try {
      ...
      seekingKnowledge.forEach((key, value) {
      ...
      await Supabase.instance.client.from('students').upsert({
        ...
        'seeking_knowledge': selectedSkills,
  ...
  Widget build(BuildContext context) {
    ...
    List<String> skillsKeys = seekingKnowledge.keys.toList();
    return Scaffold(
  ...
  Widget _buildSkillItem(String skillName) {
    return Row(
      ...
      children: [
        Checkbox(
          activeColor: const Color(0xff0f766e),
          value: seekingKnowledge[skillName],
          onChanged: (bool? value) {
            setState(() {
              seekingKnowledge[skillName] = value!;
            });
          },
        ),
